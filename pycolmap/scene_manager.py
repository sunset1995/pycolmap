...
# Updated by junyuan-fang: https://github.com/rmbrualla/pycolmap/pull/6#issuecomment-2970262506
        ...
        with open(input_file, 'r') as f:
            ...
            for line in iter(lambda: f.readline().strip(), ''):
                ...
                if is_camera_description_line:
                    ...
                    image = Image(data[-1], int(data[-2]),Quaternion( np.array(data[1:5]).astype(float)), np.array(data[5:8]).astype(float))
                else:
    ...
    def _load_points3D_txt(self, input_file):
        ...
        with open(input_file, 'r') as f:
            for line in iter(lambda: f.readline().strip(), ''):
                ...
                self.point3D_id_to_images[point3D_id] = \
                    np.array(np.array(data[8:]).astype(np.uint32)).reshape(-1, 2)
